...
## Project Overview
PowerShell 7+ binary module (C#) wrapping the GitHub Copilot SDK for .NET. Targets **net10.0** exclusively — the SDK requires `System.Text.Json 10.x`. Runs on **pwsh 7.6 preview** (.NET 10 runtime). Windows PowerShell 5.x is not supported.

**pwsh-preview** is installed at `C:\Program Files\PowerShell\7-preview\pwsh.exe` and available on PATH as `pwsh-preview`. Always use `pwsh-preview` (not `$env:LOCALAPPDATA\pwsh-preview\pwsh.exe`) when running commands that need .NET 10.
...
- `install.ps1` copies to `C:\Program Files\PowerShell\7-preview\Modules\CopilotShell` (requires admin).
...
- **mcp-wrapper/** — Separate .NET 10 console app. Combined MCP proxy with zombie daemon support:
...
- The `.csproj` uses `Version="*"` for the SDK to always pull the latest. Pin if stability is needed.
...
- Run tests with pwsh 7.6 preview: `pwsh-preview -File test.ps1`.

## Key Constraints
- .NET 10 and pwsh 7.6 preview are **required** — earlier runtimes produce `System.Text.Json` binding failures.
- `global.json` pins the SDK version with `allowPrerelease: true`.
- Cross-platform: no Windows-only APIs. `RuntimeIdentifiers`: win-x64, linux-x64, osx-x64, osx-arm64.
- Install path (`C:\Program Files\PowerShell\7-preview\Modules`) is not in default `PSModulePath` for zip-installed pwsh-preview. Scripts add it at runtime.
- Use `pwsh-preview` command (on PATH) to launch pwsh 7.6 preview. Do not use `$env:LOCALAPPDATA\pwsh-preview\pwsh.exe`.
