...
- **SessionSetupHelper.cs** — Wires custom agents, MCP servers, and tool filters onto a `SessionConfig`. Intentionally minimal: MCP servers (when a config file is given) are always attached to the session as-is (optionally wrapped), never filtered and never attached to individual agents; the session `AvailableTools` filter is only set when `-AvailableTools` is passed (otherwise the session inherits all tools); tool selectors are passed through verbatim for the runtime to resolve (`builtin:*`, `mcp:*`, `<server>/*`, `<server>/tool`, or exact wire names); an agent is pre-selected only when explicitly requested (via `-DefaultAgent` or a prompt file) — a sole custom agent is never auto-selected. Named agents supplied via `-Agent`/`-AgentNames` are resolved from `-AgentFolders` (defaulting to repo `.github/agents`, then `~/.copilot/agents`).
...
## Tool Filtering
- **Session level** (`-AvailableTools`): only applied when explicitly passed; otherwise the session inherits all tools. The values are passed to the SDK verbatim; runtime 1.0.79 resolves source-qualified selectors (`builtin:*`, `mcp:*`, `custom:*`), MCP server selectors (`<server>`, `<server>/*`, `<server>/tool`), and exact wire names. A dash glob such as `<server>-*` is a literal, not a wildcard. `-IsolatedDefaultAgent` (only when no agent is specified) restricts the session to `builtin:`-qualified entries from the isolated set (`GitHub.Copilot.BuiltInTools.Isolated`) **minus** `exit_plan_mode`/`ask_user` (keeping `send_inbox`/`context_board`), capping the built-in default agent to an orchestration-only tool set. A session-level restriction is a hard cap: it **cascades** to every agent/subagent (an agent can only narrow the session set, never widen it).
- **Agent level** (`CustomAgentConfig.Tools` / `.agent.md` `tools:`): passed through verbatim by `SessionSetupHelper`. For `.agent.md` files, `AgentFileParser` first translates VS Code Copilot tool-group names (e.g. `read/readFile` → `view`, `search` → `grep`/`glob`) to their CLI equivalents; programmatic `CustomAgentConfig.Tools` are expected to use the agent alias grammar. Exact built-in and dashed MCP wire names, bare MCP server names, `<server>/*`, and `<server>/tool` work. Session source selectors such as `mcp:*` do not. Prefer the slash forms because they preserve server identity. No MCP `tools/list` discovery is performed by CopilotShell.
...
- The module pins the stable `GitHub.Copilot.SDK` package. Types may change between releases — inspect the SDK source when docs are missing.
