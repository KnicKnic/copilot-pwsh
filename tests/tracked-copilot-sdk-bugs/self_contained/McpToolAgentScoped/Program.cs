// ============================================================================
// Self-contained repro — MCP tools not exposed via agent Tools = ["test-mcp"]
// ============================================================================
//
// A local stdio MCP server ("test-mcp", tools: alpha/beta/gamma) is attached
// to the session. A custom agent is configured with Tools = ["test-mcp"] (the
// bare MCP server name) and selected via Rpc.Agent.SelectAsync. The model
// should see the server's tools through the agent's tool scope.
//
...
var agent = new CustomAgentConfig
{
    ...
    Tools = new List<string> { McpHelper.ServerName }
};
