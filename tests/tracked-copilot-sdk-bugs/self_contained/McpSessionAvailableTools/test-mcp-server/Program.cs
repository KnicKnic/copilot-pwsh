...
// Exposes 3 tools: alpha, beta, gamma. Each tool accepts a "message"
// parameter and echoes it back. Implements just enough of the MCP JSON-RPC
// protocol to be usable with the Copilot CLI. When registered under server
// name "test-mcp", the CLI prefixes the tools as test-mcp-alpha, etc.
    ...
    switch (method)
    {
        ...
        case "tools/list":
            ...
            response = new JsonObject
            {
                ["jsonrpc"] = "2.0",
                ["id"] = id?.DeepClone(),
                ["result"] = new JsonObject { ["tools"] = toolArray }
            };
            break;
